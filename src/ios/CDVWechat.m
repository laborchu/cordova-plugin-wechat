...
- (void)openMiniPrograme:(CDVInvokedUrlCommand *)command
{
    // if not installed
    ...
    launchMiniProgramReq.userName = [message objectForKey:@"username"];  //拉起的小程序的username
    launchMiniProgramReq.path =  [message objectForKey:@"path"];    //拉起小程序页面的可带参路径，不填默认拉起小程序首页

    launchMiniProgramReq.miniProgramType =  [[message objectForKey:@"programType"] integerValue]; //拉起小程序的类型
...
- (void)jumpToWechat:(CDVInvokedUrlCommand *)command
{
    ...
    //NSURL *formatUrl = [NSURL URLWithString:[url stringByAddingPercentEncodingWithAllowedCharacters:NSUTF8StringEncoding]];
...
- (void)onResp:(BaseResp *)resp
{
    ...
    if (success)
    {
        ...
        {
            WXLaunchMiniProgramResp *wxLaunchMiniProgramResp =(WXLaunchMiniProgramResp*) resp;
            NSString *str = wxLaunchMiniProgramResp.extMsg;
...
- (WXMediaMessage *)buildSharingMessage:(NSDictionary *)message
{
    ...
    switch (type)
    {
        ...
        case CDVWXSharingTypeMini:
            ...
            ((WXMiniProgramObject*)mediaObject).userName = [media objectForKey:@"username"];
            ((WXMiniProgramObject*)mediaObject).path = [media objectForKey:@"path"];
            ((WXMiniProgramObject*)mediaObject).hdImageData = [media objectForKey:@"hdImageData"];
            break;
